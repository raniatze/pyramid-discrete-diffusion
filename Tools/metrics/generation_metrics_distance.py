...
def run_generation_metrics(args) -> None:
    ...
    reference_batch_path = (
        real_samples_cache_path / f"reference_batch_distance_{distance_threshold}m.txt"
    )
    assert (
    ...
    num_samples = len(reference_sample_paths)
    ...
    # PDFs
    args.output_dir.mkdir(parents=True, exist_ok=True)
    pdf_ref = args.output_dir / f"ref_used_for_distance_{args.distance_threshold}m.pdf"
    pdf_gen = args.output_dir / f"gen_used_for_distance_{args.distance_threshold}m.pdf"
    ...
    reference_save_path = (
        Path(args.output_dir) / f"ref_batch_distance_{distance_threshold}m.npz"
    )
    generated_save_path = (
        Path(args.output_dir) / f"samples_batch_distance_{distance_threshold}m.npz"
    )
...
def main():
    ...
    args.mode = f"distance_{args.distance_threshold}"
