...
def main(ref_batch, sample_batch, nhood_size):

    config = tf.ConfigProto(allow_soft_placement=True)  # allows DecodeJpeg to run on CPU in Inception graph
    config.gpu_options.allow_growth = True
    evaluator = Evaluator(tf.Session(config=config), nhood_sizes=nhood_size)
...
class Evaluator:
    def __init__(
        ...
        self.manifold_estimator = ManifoldEstimator(session, nhood_sizes=nhood_sizes)
...
class ManifoldEstimator:
    ...
    def __init__(
        ...
        self.eps = eps
        print(f"Using {nhood_sizes} nhood size")
...
def extract_samples(path):
    ...
    if match:
        ...
        raise ValueError(f"Could not extract number from path: {path}")


def extract_distance(path):
    match = re.search(r"_distance_(.+?)\.npz$", path)
    if match:
        return match.group(1)
    else:
        raise ValueError(f"Could not extract distance from path: {path}")
...
if __name__ == "__main__":
    ...
    if mode == "distance":
        ref_batch = f"/home/raniatze/Documents/PhD/Research/pyramid-discrete-diffusion/generated/s_1_to_s_2_50K/GenerationMetrics/ref_batch_distance_{distance_threshold}m.npz"
        samples_batch = f"/home/raniatze/Documents/PhD/Research/pyramid-discrete-diffusion/generated/s_1_to_s_2_50K/GenerationMetrics/samples_batch_distance_{distance_threshold}m.npz"
        ...
        ref_batch = f"/home/raniatze/Documents/PhD/Research/pyramid-discrete-diffusion/generated/s_1_to_s_2_50K/GenerationMetrics/ref_batch_fps_{num_samples}.npz"
        samples_batch = f"/home/raniatze/Documents/PhD/Research/pyramid-discrete-diffusion/generated/s_1_to_s_2_50K/GenerationMetrics/samples_batch_fps_{num_samples}.npz"
        ref_samples = extract_samples(ref_batch)
        generated_samples = extract_samples(samples_batch)
        assert (
            ref_samples == generated_samples
        ), f"Reference batch ({ref_samples}) and samples batch ({generated_samples}) do not match!"
    else:
        raise ValueError(f"Unknown mode: {mode}")
    print(f"Loading reference batch from {ref_batch}")
    print(f"Loading generated batch from {samples_batch}")
    main(ref_batch, samples_batch, nhood_size)
