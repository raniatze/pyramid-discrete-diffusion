...
from datasets.pritti_dataset import *

# from datasets.kitti_dataset import *

dataset_choices = {"carla", "pritti", "kitti"}
...
class DataManager:
    def __init__(self, args):
        ...
        if self.args.dataset == "carla":
            ...
            self.comp_weights = get_class_weights(self.class_frequencies).to(
                torch.float32
            )
            self.args.num_classes = 11
        elif self.args.dataset == "pritti":
            self.args.num_classes = 17
            self.comp_weights = torch.ones(self.args.num_classes, dtype=torch.float32)
        elif self.args.dataset == "kitti":
            raise NotImplementedError(
                "We are going to complete the open-source work related to the SemanticKITTI dataset."
            )

    def get_train_data(self):
        if self.args.dataset == "carla":
            train_ds = CarlaDataset(
                directory=self.args.train_data_path,
    ...
    def get_infer_data(self):
        ...
        dataloader_infer = DataLoader(
            ...
            batch_size=self.args.batch_size,
            shuffle=False,
            collate_fn=infer_ds.collate_fn,
            num_workers=self.args.num_workers,
        )
